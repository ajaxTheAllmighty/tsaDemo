...
module.exports = function (config) {
    ...
    function dropDownFilterLoaded($data, $filterFieldObj){
        ...
        Helper.getDataEx(dropDownDataLoaded, $filterFieldObj['name'], this, $filterFieldObj.dropDownNameField+' AS name,'+$filterFieldObj.dropDownCodeField + ' AS code', $filterFieldObj.dropDownView, whereClause);
    }
    ...
    function filterFieldChanged(){
        ...
        if((type == "LIST" || type == "GRD")&& !filtersDropdownArrays.hasOwnProperty(fieldName)){
            ...
            m.startComputation();
            Helper.getDataEx(dropDownFilterLoaded, filterFieldObj, this, 'FIL_USE_HIERARCHY, FIL_LOC_HIERARCHY,FIL_USE_SINGLE_HIERARCHY, FIL_USE_COMPANY', 'ST_FILTER', "WHERE (FIL_IS_PRIMARY_KEY = 1) AND (FIL_VIEW_NAME = '" +filterFieldObj['dropDownView'] + "')");
        }
    }
        ...
        switch(o.state){
            case 'default':
                switch(reportState){
                    ...
                    case 'loaded':
                        ...
                        //                m("a", {href: reportUrl, download: o.view+".xlsm", target: "_blank", class: "btn btn-primary system-button_blue", onclick: reportModalClosed}, t('okBtn', 'App'))
                        //            ])
                        //        ])
                        //    )
                        //);
            ...
            case 'loaded':
                ...
                return new Modal({
                    id: 'reportLoadingModal',
                    state: 'show',
                    header: t('loadedReportHeader', 'ReportFilterModule'),
                    content: [
                        drawActiveFilter(),
                        m("div", {class: "filter-block-container clearfix"},
                            showMainFilters()
                    ...
                    modalSizeParams: {width: '60%', height: false, padding: '5% 0 5% 0'},
                    zIndex: 1005,
                    confirmBtn: t('getReportBtn', 'ReportFilterModule'),
